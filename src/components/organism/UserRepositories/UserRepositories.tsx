...
const UserRepositories: React.FC = () => {
  ...
  /* Custom hook  */
  const [repos, isLoading, error] = useFetchUserProfile<IUserRepository[]>(
    `/${profileContext?.userProfile?.login}/repos`
  );
  /* State to filter repos */
  const [filteredRepos, setFilteredRepos] =
    useState<IUserRepository[] | null>(null);
  ...
  return (
    <article>
      ...
      <section className="reposWrapper">
        {filteredRepos && (
          <p className="grayText">{`${filteredRepos?.length} results matched`}</p>
        )}
